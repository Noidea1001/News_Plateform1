...
<div class="container-fluid px-4 py-4">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        ...
        <div class="d-flex align-items-center gap-2">
            <a href="<?= url('admin/dashboard.php') ?>" class="btn btn-outline-secondary px-3.5 py-2 fw-semibold rounded-3 text-nowrap shadow-sm">
                <span><?= __('cancel') ?></span>
            </a>
            <button type="submit" form="articleForm" class="btn btn-danger px-4 py-2 fw-semibold rounded-3 text-nowrap shadow">
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-8">
                ...
                <div class="card border-0 shadow-sm p-4 mb-4">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control form-control-lg" id="title" name="title" 
                               ...
                               placeholder="<?= e(__('title_placeholder')) ?>" required>
                    </div>
                    ...
                    <div class="card bg-light border-0 p-3 mb-3">
                        <div class="d-flex align-items-center justify-content-between mb-2">
                            ...
                            <div class="d-flex gap-1">
                                <span class="badge bg-danger text-white"><?= count($galleryList) ?> <?= __('photos') ?></span>
                                <span class="badge bg-primary text-white"><?= count($videoList) ?> <?= __('videos') ?></span>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            <div class="text-xs fw-semibold text-muted mb-1.5"><i class="bi bi-image me-1 text-danger"></i> <?= __('images_label') ?></div>
                                ...
                                <div class="d-flex flex-wrap align-items-center">
                                    <?php foreach ($galleryList as $idx => $gUrl) { ?>
                                        ...
                                        <div class="btn-group btn-group-sm me-2 mb-2 shadow-2xs" role="group">
                                            <button type="button" class="btn btn-outline-danger insert-tag-btn" data-tag="[image:<?= $imgNum ?>]" title="Insert Center">
                                                <i class="bi bi-plus-circle-fill me-1"></i>[image:<?= $imgNum ?>]
                                            </button>
                                            <button type="button" class="btn btn-outline-danger px-2 insert-tag-btn" data-tag="[image:<?= $imgNum ?>:left]" title="Float Left with text wrap">
                                                <i class="bi bi-align-start"></i> Left
                                            </button>
                                            <button type="button" class="btn btn-outline-danger px-2 insert-tag-btn" data-tag="[image:<?= $imgNum ?>:right]" title="Float Right with text wrap">
                                                <i class="bi bi-align-end"></i> Right
                                            </button>
                                            <button type="button" class="btn btn-outline-danger px-2 insert-tag-btn" data-tag="[image:<?= $imgNum ?>:full]" title="Full Width">
                        ...
                        <div>
                            <div class="text-xs fw-semibold text-muted mb-1.5"><i class="bi bi-play-btn me-1 text-primary"></i> <?= __('videos_label') ?></div>
                            <?php if (!empty($videoList)) { ?>
                                <div class="d-flex flex-wrap align-items-center">
                                    <?php foreach ($videoList as $vIdx => $vUrl) { ?>
                                        <?php $vNum = $vIdx + 1; ?>
                                        <div class="btn-group btn-group-sm me-2 mb-2 shadow-2xs" role="group">
                                            <button type="button" class="btn btn-outline-primary insert-tag-btn" data-tag="[video:<?= $vNum ?>]" title="Insert Center">
                                                <i class="bi bi-play-circle-fill me-1"></i>[video:<?= $vNum ?>]
                                            </button>
                                            <button type="button" class="btn btn-outline-primary px-2 insert-tag-btn" data-tag="[video:<?= $vNum ?>:left]" title="Float Left with text wrap">
                                                <i class="bi bi-align-start"></i> Left
                                            </button>
                                            <button type="button" class="btn btn-outline-primary px-2 insert-tag-btn" data-tag="[video:<?= $vNum ?>:right]" title="Float Right with text wrap">
                                                <i class="bi bi-align-end"></i> Right
                                            </button>
                                            <button type="button" class="btn btn-outline-primary px-2 insert-tag-btn" data-tag="[video:<?= $vNum ?>:full]" title="Full Width">
                                                <i class="bi bi-arrows-expand"></i> Full
                                            </button>
                                        </div>
                                    <?php } ?>
                    ...
                    <div class="mb-3">
                        <div class="d-flex flex-wrap justify-content-between align-items-center mb-2 gap-2">
                            <label class="form-label fw-bold text-dark mb-0 d-flex align-items-center gap-2">
                                ...
                                <span class="badge bg-light text-secondary border fw-normal"><?= __('rich_editor_badge') ?></span>
                            </label>

                            <!-- Manual Drop-Cap Toggle Switch (No bg, no border) -->
                            <div class="form-check form-switch m-0 p-0 d-inline-flex align-items-center">
                                <input class="form-check-input ms-0 me-2 cursor-pointer" type="checkbox" id="has_drop_cap" name="has_drop_cap" value="1" <?= (!empty($article['has_drop_cap'])) ? 'checked' : '' ?>>
                                <label class="form-check-label fw-bold text-dark text-xs mb-0 cursor-pointer" for="has_drop_cap" title="<?= e(__('enable_drop_cap_hint')) ?>">
                ...
                <div class="card border-0 shadow-sm p-4 mb-4">
                    ...
                    <div class="mb-3">
                        <label for="video_embed_url" class="form-label fw-semibold small text-dark"><?= __('video_embed_label') ?> <?= __('one_per_line') ?></label>
                        <textarea class="form-control font-monospace small" id="video_embed_url" name="video_embed_url" rows="2" 
                    ...
                    <div class="mb-3 p-3 bg-light rounded-3 border">
                        <label for="gallery_files" class="form-label fw-bold small text-dark d-flex align-items-center gap-2">
                            <i class="bi bi-cloud-arrow-up-fill text-danger fs-5"></i>
                            <span><?= __('upload_multi_images') ?></span>
                        </label>
                        <input type="file" class="form-control" id="gallery_files" name="gallery_files[]" multiple accept="image/jpeg,image/png,image/webp">
                ...
                <div class="card border-0 shadow-sm p-4 mb-4">
                    ...
                    <div class="mb-4 p-3 bg-light rounded-3 border">
                        <label class="form-label fw-bold text-dark d-flex align-items-center gap-2 mb-2">
                            <span><?= __('template_blueprint') ?> (ពុម្ពគំរូប្លង់) <span class="text-danger">*</span></span>
                        </label>
                        <input type="hidden" id="template_type" name="template_type" value="<?= e($article['template_type'] ?? 'standard') ?>" required>

                        <div class="blueprint-picker-grid">
                            <!-- Card 1: Standard -->
                            <div class="blueprint-picker-card <?= (!isset($article['template_type']) || $article['template_type'] === 'standard') ? 'active' : '' ?>" data-val="standard">
                            ...
                            <div class="blueprint-picker-card <?= (isset($article['template_type']) && $article['template_type'] === 'investigative') ? 'active' : '' ?>" data-val="investigative">
                                <div class="blueprint-check">&check;</div>
                            ...
                            <!-- Card 3: Opinion -->
                            <div class="blueprint-picker-card <?= (isset($article['template_type']) && $article['template_type'] === 'opinion') ? 'active' : '' ?>" data-val="opinion">
                                <div class="blueprint-check">&check;</div>
