    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 pb-3 border-bottom gap-3">
        <div>
            <div class="d-flex align-items-center gap-2 mb-1 flex-wrap">
                ...
                <span class="badge bg-danger bg-opacity-10 text-danger border border-danger border-opacity-25 rounded-pill px-2.5 py-1 text-xs text-nowrap">
                    <span class="pulsing-dot bg-danger me-1"></span> <?= __('live_feed') ?>
                </span>
            </div>
            <p class="text-muted small mb-0">
                <?= __('welcome_back') ?>, <strong><?= e($currentUser['username']) ?></strong> 
                <span class="mx-1">&bull;</span> <span class="badge bg-secondary bg-opacity-10 text-secondary border border-secondary border-opacity-25 rounded-pill text-xs text-uppercase"><?= e($currentUser['role']) ?></span>
            </p>
        </div>
    ...
    <div class="row g-4 mb-4">
        ...
        <div class="col-lg-8">
            <div class="card border-0 shadow-sm rounded-4 h-100 bg-white">
                <div class="chart-card-header d-flex flex-column flex-sm-row justify-content-between align-items-sm-center gap-2 rounded-top-4">
                    <div>
                    ...
                    <div class="btn-group btn-group-sm" role="group">
                        <button type="button" class="btn btn-outline-secondary active fw-semibold text-xs"><?= __('top_stories') ?></button>
                        <button type="button" class="btn btn-outline-secondary fw-semibold text-xs"><?= __('30_days') ?></button>
                    </div>
                </div>
            ...
            <div class="card admin-card-clean overflow-hidden">
                ...
                <div class="p-4 border-bottom bg-white">
                    ...
                    <div class="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center gap-3 mb-3">
                        <div>
                            <h5 class="fw-bold mb-1 text-dark editorial-title fs-5"><?= __('publication_repository') ?></h5>
                        ...
                        <div class="btn-group btn-group-sm flex-shrink-0" id="statusFilterTabs">
                            <button class="btn btn-dark btn-sm rounded-pill px-4 filter-tab-btn active" data-filter="all"><?= __('filter_all') ?> (<?= count($articles) ?>)</button>
                            <button class="btn btn-outline-secondary btn-sm rounded-pill px-4 filter-tab-btn" data-filter="published"><?= __('published') ?></button>
                            <button class="btn btn-outline-secondary btn-sm rounded-pill px-4 filter-tab-btn" data-filter="draft"><?= __('draft') ?></button>
                        </div>
                    </div>
        ...
        <div class="col-lg-4">
            ...
            <div class="card admin-card-clean mb-4 overflow-hidden">
                <div class="chart-card-header d-flex align-items-center justify-content-between rounded-top-4">
                    ...
                    <span class="badge bg-danger bg-opacity-10 text-danger border border-danger border-opacity-25 rounded-pill px-2 py-1 text-xs">
                        <span class="pulsing-dot bg-danger me-1"></span> <?= __('live_feed') ?>
                    </span>
                </div>
                        ...
                        <?php foreach ($recentSubscribers as $sub) { ?>
                            <div class="list-group-item py-3 px-3.5 d-flex align-items-center justify-content-between">
                                <div class="d-flex align-items-center gap-2.5">
                                    <div class="bg-danger bg-opacity-10 text-danger rounded-circle p-2 fs-6 d-flex align-items-center justify-content-center" style="width:36px; height:36px;">
                                        <i class="bi bi-envelope-check-fill"></i>
                                    </div>
                                    <div>
                                        <div class="fw-semibold text-dark small text-truncate" style="max-width: 170px;" title="<?= e($sub['email']) ?>">
                                            <?= e($sub['email']) ?>
                                        </div>
                                        <div class="text-muted text-xs">
                                            Category: <span class="fw-medium text-dark"><?= e(cat_name($sub['category_name'] ?? 'All Categories')) ?></span>
                                        </div>
                                    </div>
                                </div>
