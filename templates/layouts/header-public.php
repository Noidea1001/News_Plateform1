...
<head>
    ...
    <!-- Google Fonts: Inter + Kantumruy Pro + Noto Sans Khmer -->
            ...
            <div class="d-flex align-items-center gap-2">
                <div class="dropdown">
                    <button class="btn btn-link text-white p-0 text-decoration-none dropdown-toggle"
                            style="font-size:0.72rem; font-weight:600; opacity:0.75;"
                    ...
                    <ul class="dropdown-menu dropdown-menu-end" style="min-width:140px;">
                        <li>
                            <a class="dropdown-item <?= $currentLang === 'en' ? 'active' : '' ?>" href="<?= lang_url('en') ?>">
                                English (EN)
                            </a>
                        </li>
                        <li>
                            <a class="dropdown-item <?= ($currentLang === 'kh' || $currentLang === 'km') ? 'active' : '' ?>" href="<?= lang_url('kh') ?>">
                                ភាសាខ្មែរ (KH)
                            </a>
                        </li>
                    </ul>
...
<nav class="navbar navbar-expand-lg navbar-main sticky-top py-0" style="min-height: 60px;">
    <div class="container-fluid px-3 px-md-4 h-100">
        ...
        <a class="navbar-brand d-flex align-items-center gap-2 py-0 text-decoration-none" href="<?= url('index.php') ?>">
            <span class="brand-logo-badge" style="width:44px; height:44px; font-size:1.05rem; border-radius:2px;">NP</span>
            <div class="d-none d-sm-block">
                <div class="brand-title-text" style="font-size:1.1rem; line-height:1.2;"><?= __('site_title') ?></div>
                <div style="font-size:0.6rem; font-weight:700; color:#9ca3af; text-transform:uppercase; letter-spacing:0.1em; line-height:1;">
                    <?= __('site_tagline') ?>
                </div>
            </div>
        </a>
        ...
        <div class="d-none d-lg-block mx-3" style="width:1px; height:32px; background:#e5e7eb; flex-shrink:0;"></div>
        ...
        <div class="collapse navbar-collapse" id="mainNavbar">
            ...
            <form class="d-flex my-2 my-lg-0 mx-lg-3 position-relative" style="max-width:340px; width:100%;"
                ...
                <div class="input-group input-group-sm w-100 align-items-center" style="background:#f3f4f6; border:1px solid #e5e7eb; border-radius:2px; padding-right:4px;">
                    ...
                    <input class="form-control border-0 bg-transparent shadow-none text-xs pe-1"
                           ...
                           value="<?= e($_GET['q'] ?? '') ?>">
                    <span id="publicSearchClear" class="bootstrap-search-clear text-muted p-1 <?= empty($_GET['q']) ? 'd-none' : '' ?>" title="<?= __('clear_search') ?>">
                    ...
                    <?php foreach ($breakingNews as $bItem) { ?>
                        <a href="<?= url('article.php?slug=' . urlencode($bItem['slug'])) ?>"
                           class="text-white text-decoration-none fw-semibold me-5 text-nowrap ticker-link">
                            <span style="color:#f87171; font-weight:700;">[<?= e(cat_name($bItem['category_name'])) ?>]</span>
